...
static int Sys_PrintString(struct Interrupt_State *state) {
    ...
    if(length > 0) {
         ...
         * handler tries to print.
         * Uses printLock from screen.c for unified console synchronization. */
        {
            bool iflag = Spin_Lock_Irq_Save(&printLock);
            Put_Buf(buf, length);
            Spin_Unlock_Irq_Restore(&printLock, iflag);
        }
