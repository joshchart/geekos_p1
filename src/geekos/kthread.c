...
int nextPid() {
    ...
    /* Must disable interrupts to prevent deadlock if an interrupt
     * handler needs to allocate a PID. */
    bool iflag = Spin_Lock_Irq_Save(&pidLock);
    ret = nextFreePid++;
    Spin_Unlock_Irq_Restore(&pidLock, iflag);
    return ret;
}
...
void Detach_Thread(struct Kernel_Thread *kthread) {
    ...
    bool iflag = Spin_Lock_Irq_Save(&kthreadLock);
...
static void Reaper(ulong_t arg __attribute__ ((unused))) {
    ...
    while (true) {
        /* Use Lock_Thread_Queue which properly handles interrupt disabling */
        Mutex_Lock(&s_graveyardMutex);
        Lock_Thread_Queue(&s_graveyardQueue);
        ...
        if((kthread = s_graveyardQueue.head) == 0) {
            ...
            Unlock_Thread_Queue(&s_graveyardQueue);
            Mutex_Unlock(&s_graveyardMutex);
            Disable_Interrupts();
...
static void Tlocal_Exit(struct Kernel_Thread *curr) {
    ...
    /* Called with interrupts disabled (but no lock held).
     * Temporarily enable interrupts while running destructors
...
void Schedule_And_Unlock(Spin_Lock_t * unlock_me) {
    ...
    /* Save interrupt state from lock - this was stored when the lock was acquired
     * via Lock_List/Lock_Thread_Queue. We restore it after waking up. */
    bool saved_iflag = unlock_me->iflag;
    ...
    /* Thread has woken up - restore interrupt state to what it was before locking */
    Restore_Interrupt_State(saved_iflag);
}
 ...
 *
 * Locking: Only disables interrupts (no kernel lock needed).
 * Make_Runnable acquires run_queue_spinlock internally.
 * Schedule requires interrupts disabled but kernel lock NOT held.
 */
void Yield(void) {
...
void Exit(int exitCode) {
    ...
    iflag = Save_And_Disable_Interrupts(); /* seems less than necessary, but appeases. */
 ...
 * Interrupt state is preserved: returns with interrupts in the same
 * state as when called (enabled or disabled).
 * See the Wait_For_Key() function in keyboard.c for an example.
 */
void Wait(struct Thread_Queue *waitQueue) {
    KASSERT(waitQueue != NULL); /* try to protect against passing uninitialized pointers in */

    /* Lock_Thread_Queue disables interrupts and acquires the spinlock.
     * Schedule_And_Unlock releases the spinlock and restores interrupt state. */
    Lock_Thread_Queue(waitQueue);

    /* Get current thread while interrupts are disabled (safe to use fast path) */
    struct Kernel_Thread *current = get_current_thread(0);

    /* Add the thread to the wait queue. */
    Locked_Unchecked_Add_To_Back_Of_Thread_Queue(waitQueue, current);

    /* Find another thread to run. Releases lock and restores interrupt state. */
