...
/*
 * Resolve a full path to its mount point and filesystem-relative suffix.
 * This is a helper to consolidate the common path resolution pattern.
 * Params:
 *   path - full path to resolve
 *   pMountPoint - where to store the mount point pointer
 *   pSuffix - where to store the suffix pointer (points into original path)
 * Returns: 0 if successful, ENOTFOUND if path is invalid or not mounted
 */
static int Resolve_Path(const char *path, struct Mount_Point **pMountPoint,
                        const char **pSuffix) {
    char prefix[MAX_PREFIX_LEN + 1];

    /* Split path into prefix and suffix */
    if (!Unpack_Path(path, prefix, pSuffix))
        return ENOTFOUND;

    /* Get mount point for path */
    *pMountPoint = Lookup_Mount_Point(prefix);
    if (*pMountPoint == 0)
        return ENOTFOUND;
...
static int Do_Open(const char *path, int mode, struct File **pFile,
    ...
    rc = Resolve_Path(path, &mountPoint, &suffix);
    if (rc != 0)
        return ENOTFOUND;
...
int Rename(const char *oldpath, const char *newpath) {
    ...
    rc = Resolve_Path(oldpath, &mountPoint1, &suffix1);
    if (rc != 0)
        return rc;
    ...
    rc = Resolve_Path(newpath, &mountPoint2, &suffix2);
    if (rc != 0)
        return rc;

    /* Can only rename within a single file system */
...
int SymLink(const char *oldpath, const char *newpath) {
    ...
    /* Only resolve newpath - oldpath is stored as the symlink target */
    rc = Resolve_Path(newpath, &mountPoint, &suffix);
    if (rc != 0)
        return rc;

    if (mountPoint->ops->SymLink == 0)
        return EUNSUPPORTED;

    return mountPoint->ops->SymLink(mountPoint, oldpath, suffix);
}
